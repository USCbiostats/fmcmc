---
title: "Personalized kernel functions"
author: "George G. Vega Yon"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Personalized kernel functions}
...
The object itself is actually an R environment. If we added more parameters to `kernel_new`, we would have seen those as well. Now that we have our transition kernel, let's give it a first try and see what can we get with this new `kernel_unif` object we just created with the MCMC function.

```{r first-run}
ans <- MCMC(
  ll,                    # The log-likleihood function
  initial = max(y),      # A fair initial guess
  kernel  = kernel_unif, # Our new kernel function
...
ans <- MCMC(
  ...
  kernel  = kernel_unif, # same as before
