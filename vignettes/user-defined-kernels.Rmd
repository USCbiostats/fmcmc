...
One of the biggest benefits of the `fcmc` package is the fact that it allows you
creating personalized kernel functions. `fmcmc_kernel` objects are created with
the `kernel_new` function (a function factory), and require (at least) one
...
the `n` parameter from a binomial distribution.

## Size parameter in a binomial distribution

For this example, imagine that we are interested on learning what is the size of
a population given that we already now the prevalence of a particular phenomena,
for example, a disease. Furthermore, assume that 20\% of the population acquire
this disease, and we have a random sample from this population
$y \sim \mbox{Binom}(0.2, N)$. We don't know $N$.

Such scenario, while perhaps a bit uncommon, needs a special treatment in a
...
The object itself is actually an R environment. If we added more parameters to
`kernel_new`, we would have seen those as well. Now that we have our transition
kernel, let's give it a first try and see what can we get with this new
`kernel_unif_int` object we just created with the MCMC function.
...
package, we can use any method available for it. Let's start by plotting the
chain:
...
As you can see, the trace pf the parameter started to go up right away and then
...
We can easily do so by just passing `ans` as an starting point, since `MCMC`
will automatically take the last value of the chain as starting point of this
...
A very nice mixing (at least visually) and a posterior distribution from which
